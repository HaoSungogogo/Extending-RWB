...
my $money_total = 0;
my $dem_money_total_ind = 0;
my $rep_money_total_ind = 0;
my $money_total_ind = 0;
my $color;
my $average;
my $deviation;
my $dem_com_to_com_max=0;
my $dem_com_to_can_max=0;
my $rep_com_to_com_max=0;
my $rep_com_to_can_max=0;
my $cyclepre;
my $president;
my $partyname;
...
# height=1024 width=1024 id=\"info\" name=\"info\" onload=\"UpdateMap()\"></iframe>";
  ...
  #
my @cycles;
eval {@cycles = ExecSQL($dbuser, $dbpasswd, "select distinct cycle from cs339.committee_master order by cycle","COL");};
...
print start_form(-name=>'lookupcase'),
      p('Which cycles do you want to choose:'),
        checkbox_group(
	-name=>'cyclecheckbox',
	-values => \@cycles,
	-id=> "cycleid",
	),
 	 p('which data do you want to see:'),
        checkbox_group(
	-name=>'datacheckbox',
	-id=> "dataid",
	-values => \@choosedata,
...
if ($action eq "near") {
  ...
  eval{
    @elections = ExecSQL($dbuser,$dbpasswd,"select * from electionresults where cycle = ?","ROW",$cycle);
  }; 
   $cyclepre=$elections[0];
   $president=$elections[1];
   $partyname=$elections[2];
   print "<div id=\"colorelection\" style=\"width:100\%; height:10\%\"> cycle: $cyclepre  president: $president  Partyname:  $partyname.</div>";

  
 if($what{committees}) { 
    my ($str,$error) = Committees($latne,$longne,$latsw,$longsw,$cycle,$format);
    if($dem_money_total>$rep_money_total){
    print "<div id=\"colorcom1\" style=\"width:100\%; height:10\%\">You chose1 'Committees': The total amount of money involved by the committees in the current view is : \$$money_total\n,including \$$dem_money_total\nfor Democratic Party,the max anount of money for com_to_com is : \$$dem_com_to_com_max\n and the max amount of money for com_to_can is : \$$dem_com_to_can_max\n. \$$rep_money_total\nfor Republican Party, the max anount of money for com_to_com is : \$$rep_com_to_com_max\n and the max amount of money for com_to_can is : \$$rep_com_to_can_max\n.</div>";
    }else{
   print "<div id=\"colorcom2\" style=\"width:100\%; height:10\%\">You chose1 'Committees': The total amount of money involved by the committees in the current view is : \$$money_total\n,including \$$dem_money_total\nfor Democratic Party,the max anount of money for com_to_com is : \$$dem_com_to_com_max\n and the max amount of money for com_to_can is : \$$dem_com_to_can_max\n. \$$rep_money_total\nfor Republican Party, the max anount of money for com_to_com is : \$$rep_com_to_com_max\n and the max amount of money for com_to_can is : \$$rep_com_to_can_max\n.</div>";
    }
    if (!$error) {
      if ($format eq "table") { 
  ...
  if ($what{individuals}) {
   ...
   if($dem_money_total_ind>$rep_money_total_ind){
    print "<div id=\"colorind1\" style=\"width:100\%; height:10\%\">You chose1 'Individual': The total amount of money involved by the committees in the current view is : \$$money_total_ind\n,including \$$dem_money_total_ind\nfor Democratic Party,\$$rep_money_total_ind\nfor Republican Part</div>";
    }else{
    print "<div id=\"colorind2\" style=\"width:100\%; height:10\%\">You chose 'Individual': The total amount of money involved by the committees in the current view is : \$$money_total_ind\n,including \$$dem_money_total_ind\nfor Democratic Party,\$$rep_money_total_ind\nfor Republican Part</div>";
    }
    if (!$error) {
      if ($format eq "table") { 
  ...
  if ($what{opinions}) {
    ...
    if($color==-1){
       print "<div id=\"colorop1\" style=\"width:100\%; height:10\%\">You chose1 'Opinion': the average of opinion is: $average ; the standard deviation is: $deviation</div>";
    }
    if($color==0){
       print "<div id=\"colorop2\" style=\"width:100\%; height:10\%\">You chose1 'Opinion': the average of opinion is: $average ; the standard deviation is: $deviation</div>";
    }
    if($color==1){
       print "<div id=\"colorop3\" style=\"width:100\%; height:10\%\">You chose1 'Opinion': the average of opinion is: $average ; the standard deviation is: $deviation</div>";
    }
    if (!$error) {
      if ($format eq "table") { 
...
if ($action eq "invite-user") { 
  if (!UserCan($user,"invite-users") && !UserCan($user,"manage-users")) { 
    print h2("You don't have the required permissions to invite users");
  } else {
    my @permissions;
	...
	my @givingpermissions=param('givingpermissions');
        #my $perm = join(',',@givingpermissions);
        my $error;
	$error = Userexist($email)||beeninvited($email); 
        if ($error>0 ){
	  print " user already exists or has been invited";
        }else{
	  my $randomid = rand(10000000);
          my $invitelink = "http://murphy.wot.ece.northwestern.edu/~bxp6650/rwb/rwb.pl?act=register-user&&legallink=$randomid";
          my $subject = 'You_are_invited!';
          my $address = $email;
          my $content = "$user invite you to join RWB by $user";
          open (MAIL, "| mail -s $subject $address");;
          print MAIL "$content\n";
          print MAIL $invitelink;
          close (MAIL);
          foreach my $pms (@givingpermissions){
             eval { ExecSQL($dbuser,$dbpasswd, "insert into invitedusers (randomid,email,referer,permissions) values (?,?,?,?)",undef,$randomid,$email,$user,$pms);};
        }
	# UserInvite($email,@givingpermissions);
     	 print "E-mail sent to :   ";
	 print $email;
	 print "   permission given:";
	 print @givingpermissions;
	}
    }
  }
...
#
#  new user, giving them links and achieving register                     
#

if ($action eq "register-user") {
#legally check if the link has not been used 
	my $legallink = param('legallink');
	my @check;
	eval{@check = ExecSQL($dbuser,$dbpasswd,"select linkunique from invitedusers where randomid = ?","COL",$legallink);};
	if($check[0]){
		print "<p>illegal link</p>";
	}else{
		eval{ExecSQL($dbuser,$dbpasswd,"update invitedusers set linkunique = 1  where randomid = ?","COL",$legallink);};   
 		if (!$run) { 
      			print start_form(-name=>'Register User'),
			h2('Register User'),
			...
			hidden(-name=>'randomid',-default=>[$legallink]),
			submit,
    			end_form,
    			hr;
    		} else {
			...
			my @referer;
			my @permissions;
			eval { @email = ExecSQL($dbuser, $dbpasswd, "select email from invitedusers where randomid = ? ","COL",$randomid);};
			eval { @referer = ExecSQL($dbuser, $dbpasswd, "select referer from invitedusers where randomid = ? ","COL",$randomid);};
			eval { @permissions = ExecSQL($dbuser, $dbpasswd, "select permissions from invitedusers  where randomid = ? ","COL",$randomid);};	
			my $nemail = $email[0];
			my $nreferer = $referer[0];
			my $error;
      			$error=UserAdd($name,$password,$nemail,$nreferer);
      			if ($error) { 
				print "Can't add user";
      			} else {
#split permissions into single permission and using "giveuserperm"
				my @singlepermission = split /,/,$permissions[0];
				foreach my $spermission (@permissions){
#use giveuserperm to check if it is legal.
					my $checkpermission = GiveUserPerm($name,$spermission);
					if($checkpermission){
						print "permission failed";
					}else {
						print "giving permission $spermission successfully";
					}	
				}
...
if ($action eq "give-opinion-data") { 
  if (!UserCan($user,"give-opinion-data")) { 
    ...
    if (!$run) { 
 	 ...
 	 my ($latitude,$longitude) = (undef,undef);
    #Get the latitude and the longitude of the voter from cookie 
         if(defined($locationcookie)){
            #And then parse the cookie to get the longitude and the latitude seperately
            ($latitude,$longitude) = split(/\//,$locationcookie);
         }
	 else{
    		 print h2("Can't get location from location cookie!");
   	 }
	 my $give_choice=param('give_choice');
     	 my $color;
         if ($give_choice eq "Red"){
	    $color = -1;
	}elsif($give_choice eq "Blue"){
	    $color = 1;
	}elsif($give_choice eq "Neutral"){
	    $color = 0;	
	}
	 my $alreadygivenopinion;
	 $alreadygivenopinion = checkgivenopinion($user);
	 if($alreadygivenopinion){
	      eval{ExecSQL($dbuser,$dbpasswd,"update rwb_opinions set color = ?, latitude = ?, longitude = ? where submitter = ?", undef,$color,$latitude,$longitude,$user);};
	      print "update opinion successfully";
	 }else{
	      my $error; 
	          $error = GiveOpinionsdata($color,$user,$latitude,$longitude);
	      if($error){
		  print "Can't give opinions, because: $error";
	      }else{
...
if ($action eq "add-user") { 
  if (!UserCan($user,"add-users") && !UserCan($user,"manage-users")) { 
	...
	h2('Add User'),
      ...
      if ($error) { 
	print "2Can't add user because: $error";
      } else {
...
#
# added my sub
sub GiveOpinionsdata{
	eval { ExecSQL($dbuser,$dbpasswd, "insert into rwb_opinions (submitter, color, latitude, longitude) values (?,?,?,?)",undef,$_[1],$_[0],$_[2],$_[3]) ;};
	return $@;
}

# check if user has already given the opinion

sub checkgivenopinion{
	my ($username)=@_;
	my @col;
	eval{@col = ExecSQL($dbuser,$dbpasswd,"select count(*) from rwb_opinions where submitter = ?","COL",$username);};
	if($@){
	   return 0;
	}else {
	   return $col[0]>0; 
...
sub beeninvited{
	my ($email)=@_;
	my @col;
	eval {@col= ExecSQL($dbuser,$dbpasswd, "select count(*) from invitedusers where email=?","COL",$email);};
	if ($@) { 
	  return 0;
	}else{
          return col[0]>0;
...
while ($counter < 1) {
    eval { 
      @count = ExecSQL($dbuser, $dbpasswd, 
        "select count(*)
        from cs339.committee_master natural join cs339.cmte_id_to_geo natural join cs339.comm_to_comm
        where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<?",
        "ROW",$latsw,$latne,$longsw,$longne);
    };
    eval { 
      @count1 = ExecSQL($dbuser, $dbpasswd, 
        "select count(*)
        from cs339.committee_master natural join cs339.cmte_id_to_geo natural join cs339.comm_to_cand
        where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<?",
        "ROW",$latsw,$latne,$longsw,$longne);
    };
    $counter=$count[0]+$count1[0];
    $longne += 0.02;
    $latne += 0.02;
    $longsw -= 0.02;
    $latsw -= 0.02;
  ...
  eval {
        @dem_money = ExecSQL($dbuser, $dbpasswd,
        "select sum(transaction_amnt),max(transaction_amnt)
        from cs339.committee_master natural join cs339.cmte_id_to_geo natural join cs339.comm_to_comm 
        where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<? and cmte_pty_affiliation in ('DEM', 'Dem', 'D', 'dem', 'd')",
        "ROW", $latsw, $latne, $longsw, $longne);
    };
  eval {
        @dem_money1 = ExecSQL($dbuser, $dbpasswd,
        "select sum(transaction_amnt),max(transaction_amnt) 
        from cs339.committee_master natural join cs339.cmte_id_to_geo natural join cs339.comm_to_cand 
        where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<? and cmte_pty_affiliation in ('DEM', 'Dem', 'D', 'dem', 'd')",
        "ROW", $latsw, $latne, $longsw, $longne);
  ...
  eval {
        @rep_money = ExecSQL($dbuser, $dbpasswd,
        "select sum(transaction_amnt),max(transaction_amnt)
        from cs339.committee_master natural join cs339.cmte_id_to_geo natural join cs339.comm_to_comm 
        where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<? and cmte_pty_affiliation in ('REP', 'Rep', 'R', 'rep', 'r')",
        "ROW", $latsw, $latne, $longsw, $longne);
    };
  eval {
        @rep_money1 = ExecSQL($dbuser, $dbpasswd,
        "select sum(transaction_amnt),max(transaction_amnt)
        from cs339.committee_master natural join cs339.cmte_id_to_geo natural join cs339.comm_to_cand 
        where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<? and cmte_pty_affiliation in ('REP', 'Rep', 'R', 'rep', 'r')",
        "ROW", $latsw, $latne, $longsw, $longne);
...
eval {
        @total1 = ExecSQL($dbuser, $dbpasswd,
        "select sum(transaction_amnt) 
        from cs339.committee_master natural join cs339.cmte_id_to_geo natural join cs339.comm_to_cand 
        where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<?",
        "ROW", $latsw, $latne, $longsw, $longne);
    };
$dem_money_total = $dem_money[0]+$dem_money1[0];
$rep_money_total = $rep_money[0]+$rep_money1[0];
$money_total = $total[0]+$total[0];
$dem_com_to_com_max = $dem_money[1];
$dem_com_to_can_max = $dem_money1[1];
$rep_com_to_com_max = $rep_money[1];
$rep_com_to_can_max = $rep_money1[1];
	#print "<h3>You chose 'Committees': The total amount of money involved by the committees in the current view is : \$$amount\n,including \$$dem_money[0]\nfor Democratic Party,\$$rep_money[0]\nfor Republican Party</h3>";
...
while (@count[0] < 1) {
    eval { 
      @count = ExecSQL($dbuser, $dbpasswd, 
        "select count(*)
        from cs339.committee_master natural join cs339.ind_to_geo natural join cs339.individual
        where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<?",
        "ROW",$latsw,$latne,$longsw,$longne);
    };
  ...
  eval {
	...
	#print "<h3>You chose 'Individual': The total amount of money involved by the committees in the current view is : \$$amount\n,including \$$dem_money[0]\nfor Democratic Party,\$$rep_money[0]\nfor Republican Party</h3>";
$dem_money_total_ind = $dem_money[0];
$rep_money_total_ind = $rep_money[0];
$money_total_ind = $total[0];
...
#        $rows[2]="neutral";
#  };
  my @givingopinioncolor;
  eval {
    @givingopinioncolor = ExecSQL($dbuser, $dbpasswd, "select avg(color),stddev(color) from rwb_opinions where latitude>? and latitude<? and longitude>? and longitude<?",undef,$latsw,$latne,$longsw,$longne);
  };
  my $data = $givingopinioncolor[0];
     $average = ${$data}[0];
     $deviation = ${$data}[1];
  if ($average > 0.5){
    $color = 1;
